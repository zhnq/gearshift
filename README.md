...
**v0.3.1** · 一个 Windows 桌面「场景切换」工具：右键任务栏托盘图标，一键在**游戏 / 办公 / 会议**等场景间切换，自动关闭/启动程序、开关系统代理、切电源计划，切回时自动恢复。

## 直接运行（发布版）

从 [**Releases**](https://github.com/zhnq/gearshift/releases/latest) 下载 `GearShiftSetup-0.3.1-win-x64.exe` 并运行安装器。
...
完整 UI 原型（16 个界面 × 浅/深 = 32 画板，WinUI + Fluent Design）用 Figma 设计。
...
- [x] **自动化与性能**：程序/前台窗口/电源/Wi-Fi/时间/外接显示器触发、临时场景自动恢复、桌面快捷方式、Ctrl+Alt+1..9 热键、执行预览/重试/历史、并发进程动作与耗时记录、程序目录后台缓存。
- [x] **工作环境动作**：显示器拓扑（扩展/复制/仅内置/仅外接）、默认播放设备、窗口布局记录与恢复；程序选择支持 UWP 与 Steam 库发现。
...
发布包采用 Inno Setup 的 LZMA2/Ultra 固实压缩，安装到当前用户的应用目录，提供开始菜单、可选桌面快捷方式、卸载项和静默更新。它不内置 .NET/Windows App SDK 运行时，以保持安装器较小。

正式发布可选 Authenticode 签名：在 GitHub Actions 仓库 Secrets 中配置
`WINDOWS_SIGNING_CERTIFICATE_BASE64`（PFX 的 Base64 内容）和
`WINDOWS_SIGNING_CERTIFICATE_PASSWORD`。未配置证书时仍可构建，但 Release 会明确保留为未签名版本。
每次 Release 同时发布 `SHA256SUMS.txt`；应用内更新还会校验 GitHub API 返回的资产摘要。
